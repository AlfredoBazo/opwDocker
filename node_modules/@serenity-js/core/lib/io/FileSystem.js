...
class FileSystem {
    constructor(root, fs = gracefulFS, os = nodeOS, directoryMode = parseInt('0777', 8) & (~process.umask())) {
        this.root = root;
        this.fs = fs;
        this.os = os;
    ...
    stat(relativeOrAbsolutePathToFile) {
        const stat = util_1.promisify(this.fs.stat);
        return stat(this.root.resolve(relativeOrAbsolutePathToFile).value);
    }
    remove(relativeOrAbsolutePathToFileOrDirectory) {
        const stat = util_1.promisify(this.fs.stat), unlink = util_1.promisify(this.fs.unlink), readdir = util_1.promisify(this.fs.readdir), rmdir = util_1.promisify(this.fs.rmdir);
        ...
        return stat(absolutePath.value)
            ...
            : readdir(absolutePath.value).then(entries => Promise.all(entries.map(entry => this.remove(absolutePath.join(new Path_1.Path(entry))))).then(() => rmdir(absolutePath.value))))
            .then(() => void 0)
            .catch(error => {
    ...
    rename(source, destination) {
        const rename = util_1.promisify(this.fs.rename);
