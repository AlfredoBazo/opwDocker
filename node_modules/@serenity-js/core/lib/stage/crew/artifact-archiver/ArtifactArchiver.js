...
class ArtifactArchiver {
     ...
     */
    constructor(fileSystem, stage = null) {
    ...
    fileNameFor(prefix, artifactName, artifact, extension) {
        ...
        const urlFriendly = (name) => name.toLocaleLowerCase()
            .replace(/[^a-z0-9.-]/g, '-');
    ...
    archivisationAnnouncement(evt, relativePathToArtifact) {
        return (absolutePath) => {
            if (evt instanceof events_1.ActivityRelatedArtifactGenerated) {
                this.stage.announce(new events_1.ActivityRelatedArtifactArchived(evt.sceneId, evt.activityId, evt.name, evt.artifact.constructor, relativePathToArtifact));
            }
            else if (evt instanceof events_1.ArtifactGenerated) {
                this.stage.announce(new events_1.ArtifactArchived(evt.sceneId, evt.name, evt.artifact.constructor, relativePathToArtifact));
            }
        };
    }
}
