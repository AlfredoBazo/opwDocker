...
class StandardisedCapabilities {
    ...
    browserVersion() {
        return this.get(caps => caps.get('version'), caps => caps.get('browserVersion'), caps => caps.has('deviceManufacturer') && caps.has('deviceModel')
            ? `${caps.get('deviceManufacturer')} ${caps.get('deviceModel')}`
            : undefined, caps => caps.has('mobile') && caps.get('mobile').version).
            then(version => this.get(caps => !!caps.get('mobileEmulationEnabled') && '(mobile emulation)').then(suffix => [
            version,
            suffix,
        ].filter(_ => !!_).join(' ')));
    }
