...
class Cookie {
    ...
    static domainOf(cookieName) {
        return core_1.Transform.the(new CookieDetails(cookieName), details => details && details.domain)
            .as(`the domain of the "${cookieName}" cookie`);
    }
    static isHTTPOnly(cookieName) {
        return core_1.Transform.the(new CookieDetails(cookieName), details => details && !!details.httpOnly)
            .as(`the HTTP-only status of the "${cookieName}" cookie`);
    }
    static isSecure(cookieName) {
        return core_1.Transform.the(new CookieDetails(cookieName), details => details && !!details.secure)
            .as(`the "secure" status of the "${cookieName}" cookie`);
    }
    static expiryDateOf(cookieName) {
        return core_1.Transform.the(new CookieDetails(cookieName), details => {
