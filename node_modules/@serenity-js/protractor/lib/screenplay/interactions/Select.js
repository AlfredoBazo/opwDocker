...
class SelectValues {
    ...
    performAs(actor) {
        return Promise.all(this.values.map(value => actor.answer(value)))
            .then(flatten)
            .then(values => {
            const hasRequiredValue = (option) => option.getAttribute('value').then(value => !!~values.indexOf(value)), select = (option) => option.click();
...
class SelectOptions {
    ...
    performAs(actor) {
        return Promise.all(this.values.map(value => actor.answer(value)))
            .then(flatten)
            .then(values => {
            const hasRequiredText = (option) => option.getText().then(value => !!~values.indexOf(value)), select = (option) => option.click();
            return promiseOf_1.promiseOf(withAnswerOf_1.withAnswerOf(actor, this.target, (element) => element.all(protractor_1.by.css('option'))
...
function optionsToSelect(criterion) {
    const isAlreadySelected = (option) => option.isSelected(), xor = (first, second) => first !== second;
